...
export async function getPublicBusiness(slug: string) {
  ...
  const { data } = await supabase
    .from("businesses")
    .select(
      "id, slug, name, timezone, logo_path, brand_color, phone, phone_country_code, address, status, auto_confirm_bookings, min_booking_notice_hours, max_booking_days, cancellation_notice_hours",
    )
    .eq("slug", slug)
    .maybeSingle();
